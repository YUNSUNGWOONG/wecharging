...
story += bullet([
    ...
    "헬스 메트릭(HTTP /metrics + Grafana 보드), OTA 업데이트 FSM.",
    "연동 — Gazebo RGB-D 영상 특징 검출 프론트엔드(영상→자세→서보→도킹 폐루프).",
    "전체 자동화 테스트 149개 전부 통과. (#1 RFID 실측 모델만 실물 장비 필요로 보류.)",
])
...
story += [P("3.1 단위 테스트", h2),
          P("ROS의 launch_testing 플러그인이 최신 pytest와 충돌해 테스트 수집이 깨지는 "
            ...
            "추가하여 현재 총 <b>149개 테스트 전부 통과</b>한다.")]
...
story += [P("영상 특징 검출 프론트엔드(detector.py): Gazebo RGB-D 영상에서 어두운 소켓 4코너를 "
            "검출해 깊이·내부파라미터로 3D 역투영 → 자세 추정으로 연결. 합성 RGB-D로 영상→검출→"
            "자세→서보→도킹 전 과정을 검증(테스트 6개).", cap)]
...
story += bullet([
    ...
    "<b>Prometheus 메트릭 + Grafana</b> — 플릿·안전 상태를 노출 형식으로 렌더하고 stdlib HTTP "
    "/metrics 엔드포인트로 서빙(라이브 스크레이프), 7패널 Grafana 보드 포함.",
    "<b>OTA 업데이트 FSM</b> — 다운로드→검증→스테이징→활성화, 체크섬 불일치/헬스 실패 시 "
    "롤백. 검증 버전은 헬스체크 통과 시에만 커밋(벽돌 방지).",
])
story += [P("안전 테스트 38개(하중 8·E-stop 9·메트릭 7·OTA 9·HTTP+Grafana 5). 위험 감지 → "
            "전원 차단이 모듈 간 연결되어 검증된다.", cap)]
...
story += [table([
    ...
    ["skyvolt_safety/ (패키지)", "신규", "하중·E-stop·메트릭(HTTP/Grafana)·OTA + 38 테스트"],
...
for line in [
    "cd skyvolt_mvp",
    "./scripts/run_tests.sh            # all tests (149)",
...
story += bullet([
    ...
    "Phase 3 — 접촉 물리 sim(F/T 실데이터), 실제 카메라 영상 프레이밍·검출 튜닝. [sim]",
    "Phase 4 — HW GPIO(E-stop) 결선, OTA 전송 계층, Prometheus·Grafana 배포. [HW/인프라]",
])
