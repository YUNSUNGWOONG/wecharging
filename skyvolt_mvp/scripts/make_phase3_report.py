...
story += [P("4개 작업의 핵심 IP를 모두 구현. skyvolt_manipulator + skyvolt_vision, 테스트 "
            "40개, 저장소 전체 105개 통과. 검출 → 자세추정 → 정렬·삽입·도킹 → 힘 기반 안전감시가 "
...
story += bullet([
    "<b>Gazebo 연동</b> — RGB-D 센서 + 특징 검출 프론트엔드, F/T 센서, 매니퓰레이터 조인트 "
    "구동. (핵심 IP는 모두 완료, 이제 sim에 센서를 연결.)",
])
...
story += [table([
    ...
    ["skyvolt_vision/ (패키지)", "신규", "충전구 자세 추정 + 비주얼 서보잉 + 18 테스트"],
...
story += codeblock([
    "cd skyvolt_mvp",
    "./scripts/run_tests.sh    # all tests (105, incl. 22 manip + 18 vision)",
